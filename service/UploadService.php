...
class UploadServices {
    ...
    public static function getHttpPostFileVariable(){

    }

    public function upload_file_now($field) {

        $config_field = USER_FILE_UPLOAD_FIELD_NAME;
        $config_max_file_size = MAX_FILE_SIZE_LIMIT;

        /**
         * Remember: ALL VARIABLES USED HERE ARE PROPERTY OF UPLOAD SERVICE CLASS
         *
         */
        $this->user_file_upload_html_field = isset($field) ? $field :  $config_field ;

        $this->maxFileSizeAllowed = $config_max_file_size  ;

        $this->postFileName = isset($_FILES[$field]['name'])? $_FILES[$field]['name'] : die('Error , no file for upload is detected');

        //clean file name
        $this->postFileName = $this->clean_file_name($this->postFileName);

        $this->postFileSize = isset($_FILES[$field]['size'])? $_FILES[$field]['size']: die('Error , file size is unknown') ;

        $this->postFileType = isset($_FILES[$field]['type'])? $_FILES[$field]['type'] : die('Error , file is type can\'t be detected') ;

        $this->postFileTempName  = isset($_FILES[$field]['tmp_name'])?  $_FILES[$field]['tmp_name']: null;
...
//
        ...
        if ( in_array( $this->postFileType,  $this->allowed_file_type_headers )
            ...
            if ( $_FILES[ $this->user_file_upload_html_field]["error"] > 0)  {
                ...
                //RENAMING NEW VIDEO File
                //TODO:You can merge user id with video name here

                $extract_temp = explode(".",$_FILES["file"]["name"]);
                 ...
                 */
                $new_file_name = $get_prefix. rand(1,99999) . '.' .end($extract_temp);

                $this->postFileName = $new_file_name ;

                $complete_file_path_server = DEFAULT_VIDEO_STORE_LOCATION."/" . $this->postFileName ;

                 /**
                 * DO PATH CORRECTION BASED ON OS
                 *
                 */
                ...
                */
                ...
                if (file_exists($complete_file_path_server)) {
                    echo( $this->postFileName . " already exists. ");
                } else  {
                     ...
                     */
                        ...
                        if (is_dir(DEFAULT_VIDEO_STORE_LOCATION)) {
                            ...
                            echo("Stored in: " .DEFAULT_VIDEO_STORE_LOCATION. "/" . $this->postFileName );
    ...
    public function ffmepg_generate_thumbnails($src_video_path,$debug=false ,$overwrite = true) {
       ...
       // $fileExtension = pathinfo($src_video_path , PATHINFO_EXTENSION);
        ...
        foreach (VideoModel::get_thumbnail_sizes() as $size) {
           ...
           $thumb_output_file = DEFAULT_VIDEO_STORE_LOCATION .'/'. "thumb_" . $size . "_" .$video_name[0] . ".jpg";
